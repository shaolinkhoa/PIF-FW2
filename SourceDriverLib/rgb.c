...
RGBColorSet(volatile uint32_t * pui32RGBColor)
{
    ...
    for(ui32Index=0; ui32Index < 3; ui32Index++)
    {
     ...
     TimerMatchSet(RED_TIMER_BASE, RED_TIMER, ui32Color[RED]);
     TimerMatchSet(GREEN_TIMER_BASE, GREEN_TIMER, ui32Color[GREEN]);
     TimerMatchSet(BLUE_TIMER_BASE, BLUE_TIMER, ui32Color[BLUE]);
}
...
RGBBlinkRateSet(float fRate)
{
    ...
    else
    {
        ...
        ui64Load = (uint64_t) (((float)SysCtlClockGet()) / (fRate * 2.0f));
         TimerLoadSet(WTIMER5_BASE, TIMER_B, ui64Load);
